...
app.get("/:version/:mod{^p5.(?:([a-zA-Z0-9_-]+)\.)?js$}", async (c) => {
  ...
  return match<string | undefined>(moduleType)
    ...
    .with(
      P.when((i) => allModules.includes(i)),
      async () => {
        console.log(pjson.exports);
        console.log(nodeModulesPath, pjson.exports[`./${moduleType}`]);
    ...
    .with("custom", async () => {
      ...
      const input = [...defaultModules, path.join(nodeModulesPath, `./dist/core/init.js`)];
